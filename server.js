...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // Admin special case
    const isAdmin = username.toLowerCase() === 'adminmaik';
...
function makeParty(leaderSocket, leaderName, mode, diff) {
  let code;
...
function spawnEnemy(party, boss=false) {
  ...
  if (boss) {
    const hp = Math.round((400 + party.wave*90)*cfg.bossHpMul);
...
function startPartyGame(party) {
  ...
  io.to(party.code).emit('game:start', { mode: party.mode, diff: party.diff });
...
function tickPartyGame(party) {
  ...
  if (!party.bossActive && party.bossTimer >= 6000) {
    party.bossTimer = 0;
    party.bossWarn = true;
...
io.on('connection', socket => {
  ...
  // ── PARTY EVENTS ──────────────────────────────────────────
  socket.on('party:create', ({ mode, diff }) => {
    ...
    const party = makeParty(socket, socketUser.username, mode || 'coop', diff || 'normal');
  ...
  socket.on('party:startGame', () => {
    const party = partyOf(socket.id);
    if (!party || party.leader !== socket.id) return socket.emit('error', 'Nur der Leader kann das Spiel starten.');
    if (party.members.length < 2) return socket.emit('error', 'Mind. 2 Spieler nötig.');
